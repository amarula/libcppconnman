...
TEST(Connman, getServs) {
    bool called = false;
    {
        ...
        manager->onTechnologiesChanged([main_tid = thread_bundle.main_tid,
        ...
        manager->onServicesChanged(
            [&called, main_tid = thread_bundle.main_tid,
             loop_tid = thread_bundle.loop_tid](const auto& services) {
                called = true;
                const auto callback_tid = std::this_thread::get_id();
                EXPECT_NE(callback_tid, main_tid);
                EXPECT_NE(callback_tid, loop_tid);
...
TEST(Connman, ConnectWifi) {
    ...
    bool called_request_input = false;
    {
        ...
        manager->onRequestInputPassphrase(
            [&called_request_input, main_tid = thread_bundle.main_tid,
             loop_tid = thread_bundle.loop_tid](auto service) -> auto {
                const auto callback_tid = std::this_thread::get_id();
                EXPECT_NE(callback_tid, main_tid);
                EXPECT_NE(callback_tid, loop_tid);
